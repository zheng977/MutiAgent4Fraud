...
plt.rcParams.update({'font.size': 24})  # 全局字体变大
...
def visualize_multiple_tweet_stats(data_paths: Union[Dict, str], 
    ...
    confidence_interval = stats.t.interval(
        0.95, all_stats.shape[0] - 1, loc=avg_stats, scale=std_stats/np.sqrt(all_stats.shape[0]))
...
def visualize_comparison(data_parent_dir: str, save_path: str = None, 
    ...
    single_filter = 'good_alone'
    decentralized_filter = 'good_bad_random'
    centralized_filter = 'good_bad_member'
    only_good_filter = 'only_good'

    # Define colors for each configuration
    colors = {
        # 'Single': ['blue', 'lightblue'],
        'With Peer Reflections': ['green', 'lightgreen'],
        'Without Peer Reflections': ['red', 'lightcoral'],
        # 'Only Good': ['purple', 'lightpurple']
    }
    ...
    configs = [
        # ('Single', single_filter, colors['Single']),
        ('With Peer Reflections', decentralized_filter, colors['With Peer Reflections']),
        ('Without Peer Reflections', centralized_filter, colors['Without Peer Reflections']),
        # ('Only Good', only_good_filter, colors['Only Good'])
    ]
...
def save_tweet_stats_visualization(save_paths: List[str], data_parent_dir: str):
    ...
    for save_path in save_paths:
        if "/both" in save_path:
            ...
            title = 'Comparison of Single, Centralized and Decentralized Groups'
...
def visualize_model_setting_comparison(
    model_dirs: Dict[str,str],
    filter_str: str,
    save_path: str = None,
    title: str = 'Model Comparison on Same Setting',
    show_likes: bool = True,
    show_reposts: bool = True,
    show_good_comments: bool = True,
    show_bad_comments: bool = True,
    ...
    """
    比较不同模型在相同设置(filter_str)下的推文统计数据随时间的变化，
    并绘制平均值及95%置信区间。

    Args:
        model_dirs (Dict[str,str]): 键为模型名称，值为该模型保存 .npy 文件的文件夹路径。
        filter_str (str): 用于筛选该设置对应文件名的子串。
        save_path (str, optional): 若提供，则保存图到此路径。
        title (str): 图标题。
        show_likes, show_reposts, show_good_comments, show_bad_comments (bool):
            是否绘制对应的指标曲线。
        plot_now (bool): 是否立即 plt.show()。

    Returns:
        fig, ax: matplotlib figure 和 axis 对象，方便后续调整。
    """
    # 定义要绘制的指标
    metrics = [
        ('Likes',          show_likes,        0, '--'),
        ('Reposts',        show_reposts,      1, '-.'),
        ('Good Comments',  show_good_comments,2, ':'),
        ('Bad Comments',   show_bad_comments, 3, '-')
    ]
    ...
    for model_name, model_dir in model_dirs.items():
        # 收集所有符合 filter_str 的 .npy 文件
        files = [
            os.path.join(model_dir, f)
            for f in os.listdir(model_dir)
            if f.endswith('.npy') and filter_str in f
        ]
        if not files:
            print(f"[Warning] 模型 '{model_name}' 在路径 '{model_dir}' 下没有匹配 '{filter_str}' 的文件，跳过。")
            continue

        # 读取所有实验数据
        all_stats = [np.load(fp) for fp in files]
        all_stats = np.array(all_stats)  # shape = [num_runs, num_timesteps, 4]

        # 计算平均和标准差
        avg = np.mean(all_stats, axis=0)      # shape = [timesteps, 4]
        std = np.std(all_stats, axis=0)       # shape = [timesteps, 4]
        n_runs = all_stats.shape[0]

        # 95% CI, 返回 (low, high)，都是 shape = [timesteps, 4]
        ci_low, ci_high = stats.t.interval(
            0.95,
            df = n_runs - 1,
            loc = avg,
            scale = std / np.sqrt(n_runs)
        )

        # 绘制每个指标
        num_timesteps = avg.shape[0]
        timesteps = np.arange(1, num_timesteps + 1)

        for metric_name, do_show, idx, linestyle in metrics:
            if not do_show:
                continue
...
if __name__ == "__main__":
    ...
    result_path = r"results/defense/sharedref_ablation_ban"
    ...
    save_paths = [
        ...
        f"results/defense/sharedref_ablation_ban/both_reposts_{result_path_str}.png",
        f"results/defense/sharedref_ablation_ban/both_likes_{result_path_str}.png",
        f"results/defense/sharedref_ablation_ban/both_good_comments_{result_path_str}.png",
        f"results/defense/sharedref_ablation_ban/both_bad_comments_{result_path_str}.png"
    ]
    save_paths = [_+".png" for _ in save_paths]
    if False:
        save_tweet_stats_visualization(save_paths=save_paths, data_parent_dir=result_path)

    if True:
        model_dirs = {
            "baseline": r"results\misinformation\5.06_qwen7b_baseline",
            "ablation": r"results\misinformation\5.07_qwen7b_ablation"
        }
        # 比如所有文件名中都含有 "post_stats_data_test_1000_good_bad"
        decentralized_filter = "good_bad_random"
        centralized_filter = "good_bad_member"
        single_filter = "good_alone_bad"
        only_good_filter = "only_good"
        fig, ax = visualize_model_setting_comparison(
            model_dirs=model_dirs,
            filter_str=decentralized_filter,
            save_path=r"results\misinformation\compare_models.pdf",
            title="decentralized qwen7b ablation",
            show_likes=True,
            show_reposts=True,
            show_good_comments=False,
            show_bad_comments=False,
