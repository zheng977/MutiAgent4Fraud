#!/bin/bash
#SBATCH --job-name=vllm-4-dp
#SBATCH --partition=AI4Good_L1_p        
#SBATCH --ntasks=4 # 任务总数
#SBATCH --nodes=1
#SBATCH --gpus-per-task=1   # 每个任务1张卡
#SBATCH --cpus-per-task=8   # 每个任务cpu数量
#SBATCH --output=/dev/null  # 丢弃标准输出流
#SBATCH --error=log/vllm_dp_%j.log # 对标准错误流log

# ================= 用户配置区域 =================
MODEL_NAME="/mnt/petrelfs/renqibing/workspace/models/qwen"

PORT_NUM_PER_INSTANCE=12
# 实例端口
MAIN_PORTS=(
    40000
    41000
...
run_vllm() {
    local port=$1
    srun --gres=gpu:1 --overlap --ntasks 1 python -m vllm.entrypoints.openai.api_server \
        --model $MODEL_NAME \
        --dtype auto \
        --port $port \
        --trust-remote-code \
...
echo "全部可用端口: $(format_list $(printf "%s\n" "${MAIN_PORTS[@]}" "${ALL_PORTS[@]}" | shuf))" >&2
echo "==========================================" >&2
